...
> **وضعیت پیاده‌سازی:** در شاخهٔ فعلی هیچ ماژول Core/Infra/UI با نام «BuildMentorPool» یا فیلدی به نام `mentor_status` پیاده‌سازی نشده است؛ بخش حاضر نیازمندی آینده را مستند می‌کند تا قبل از ادعای «پیاده‌سازی شده» کد و تست مرتبط اضافه شوند.
...
- **Core Responsibilities:**
  - (Future work) مرحلهٔ صریح «BuildMentorPool» قبل از ساخت ماتریس اجرا خواهد شد تا فقط پروفایل‌های `ACTIVE` وارد شوند و پروفایل‌های `FROZEN` ظرفیت صفر تلقی شده و حذف گردند؛ تا زمان اضافه‌شدن این مرحله در Core، اپراتور باید به‌صورت دستی فهرست پشتیبان‌ها را کنترل کند.
  - محدودیت‌های `RESTRICTED_*` در صورت تعریف، به‌عنوان قید eligibility هنگام ساخت ماتریس اعمال می‌شوند نه به‌عنوان استثناء رتبه‌بندی؛ ۶ کلید join و سیاست رتبه‌بندی ثابت می‌مانند. (این قید نیز نیازمند پیاده‌سازی آتی در Core است.)
- **UI Responsibilities:**
  - (Future work) یک پنل PySide6/CLI برای «مدیریت استخر پشتیبان‌ها» نمایش داده می‌شود تا اپراتور بتواند وضعیت mentor_status را مشاهده و میان `ACTIVE`/`FROZEN` (و حالت‌های محدود) جابه‌جا کند. در نسخهٔ کنونی UI چنین پنلی وجود ندارد و باید طبق همین مشخصات توسعه یابد.
