...
| POOL_02 | Mentor pool governance UI (enable/disable mentors/managers) | implemented | — | CLI: `_ui_overrides` → `mentor_pool_overrides` + `mentor_pool_manager_overrides` in `app/infra/cli.py` (build + allocate) | `app/ui/main_window.py` (toolbar + allocate + matrix tabs), `app/ui/mentor_pool_dialog.py`, `app/ui/mentor_pool_model.py` | `tests/ui/test_mentor_pool_dialog.py`, `tests/ui/test_main_window_mentor_pool_integration.py`, `tests/infra/test_mentor_pool_governance_plumbing.py` | Governance dialog now groups manager→mentor with cascade + search; matrix tab exposes Inspactor governance before build. |
