...
| POOL_01 | Mentor pool governance (status-driven filtering) | implemented | `app/core/allocation/mentor_pool.py` (`compute_effective_status`, `apply_mentor_pool_governance`) | CLI wiring in `app/infra/cli.py` (`_apply_mentor_pool_overrides`) | — | `tests/core/allocation/test_mentor_pool_governance.py`; `tests/infra/test_mentor_pool_governance_plumbing.py` | Core filters mentors by policy status and optional overrides; attrs report totals/removed/override count. |
| POOL_02 | Mentor pool governance UI (enable/disable mentors/managers) | implemented | — | `_ui_overrides` plumbing in `app/infra/cli.py` | `app/ui/main_window.py`; `app/ui/mentor_pool_dialog.py`; `app/ui/models.py` | `tests/ui/test_mentor_pool_dialog.py`; `tests/ui/test_main_window_mentor_pool_integration.py` | Allocate tab + toolbar expose governance dialog; overrides forwarded to Infra for per-run filtering. |
