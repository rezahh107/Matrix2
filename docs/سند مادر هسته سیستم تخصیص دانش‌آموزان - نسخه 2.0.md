...
- تابع `apply_mentor_pool_governance` در Core آداپتور رسمی روی `compute_effective_status`/`filter_active_mentors` است و تنها نقطهٔ مصرف UI/CLI برای اعمال override نوبتی باقی می‌ماند تا منطق حاکمیت تکرار نشود.
...
- برای ثبت متادیتای اجرا در حالت آفلاین، Infra یک لایهٔ SQLite (`app/infra/local_database.py`) دارد که UUID اجرا، نسخه‌های policy/ssot، مسیر و هش ورودی‌ها، خلاصهٔ QA و KPI تاریخچه را ذخیره می‌کند؛ CLI/UI می‌توانند مسیر فایل را با `--local-db` تعیین یا با `--disable-local-db` غیرفعال کنند.
- Schema اولیه شامل جداول `runs`, `run_metrics`, `qa_summary` است و ایجاد آن idempotent می‌باشد.
