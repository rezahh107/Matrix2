...
- **یادآوری برنامه‌ریزی‌شده:** این مرحله هنوز در کد مستقر نشده و به‌عنوان الزام آینده مستند شده است تا QA/توسعه بداند که رفتار فعلی Core/Infra فاقد BuildMentorPool و mentor_status است.
...
- منطق فیلتر:
  1. حذف کامل تمام ردیف‌هایی که `mentor_status = FROZEN` دارند؛ این افراد حتی به مرحلهٔ محاسبهٔ ظرفیت/alias نمی‌رسند.
  2. در صورت وجود حالات `RESTRICTED_*`, شناسه‌های محدودشده در استخر باقی می‌مانند ولی constraint آن‌ها به قید eligibility در مرحلهٔ ماتریس تبدیل می‌شود (مثلاً فقط یک مدرسه یا کانال).
...
- ستون/فیلد وضعیت باید به خروجی trace و summary اضافه شود تا در هر اجرای QA بتوان نشان داد که هر حذف/ورود از استخر با استناد به Policy انجام شده است؛ Core صرفاً پیام «FROZEN (policy_version=1.0.3)» یا مشابه را از دادهٔ ورودی تکرار می‌کند و هیچ نامی را در کد نمی‌نویسد.
...
- Infra پس از دریافت `QaReport` باید ورک‌بوک اعتبارسنجی Excel تولید کند که شیت summary و شیت‌های تفکیکی (`students_per_mentor`, `school_binding_issues`, `allocation_capacity`, `student_counts`, `join_keys`, `meta`) را شامل شود تا اپراتور بتواند خروجی QA را حتی در صورت شکست قوانین مرور کند.
...
- UI یک دیالوگ «حاکمیت استخر منتورها» فراهم می‌کند تا اپراتور بتواند برای هر اجرای تخصیص منتورها/مدیران را فعال یا غیرفعال کند و این overrideها از مسیر UI→Infra→Core به فیلتر استخر اعمال شود بدون آن‌که semantics Policy تغییر کند.
