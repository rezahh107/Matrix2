...
def test_stu_02_keeps_zero_padded_and_mixed_ids() -> None:
    """شناسه‌های صفر-پد شده یا ترکیبی نباید در شمارش حذف شوند."""

    allocation = pd.DataFrame(
        {
            "mentor_id": ["000123", "000123", "EMP-3", "EMP-3"],
            "student_id": ["S1", "S2", "S3", "S4"],
        }
    )
    inspactor = pd.DataFrame(
        {
            "mentor_id": ["000123", "EMP-3"],
            "expected_student_count": [3, 1],
        }
    )
    ...
    assert not result.passed
    assert any(
        v.details == {"mentor_id": "000123", "expected": 3, "assigned": 2}
        for v in result.violations
    )
